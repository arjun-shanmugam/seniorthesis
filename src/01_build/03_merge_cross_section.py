...
OUTPUT_DATA_UNRESTRICTED = "/Users/arjunshanmugam/Documents/GitHub/seniorthesis/data/03_cleaned/cross_section_unrestricted.csv"
OUTPUT_DATA_RESTRICTED = "/Users/arjunshanmugam/Documents/GitHub/seniorthesis/data/03_cleaned/cross_section_restricted.csv"
VERBOSE = True
...
tax_parcels_gdf.to_csv("~/Desktop/tax_parcels_gdf.csv")
merged_df = tax_parcels_gdf.merge(pd.read_csv(INPUT_DATA_ASSESSMENT_VALUES),
                                  how='inner',
                                  right_on=['LOC_ID', 'FY'],
                                  left_on=['LOC_ID', 'next_fiscal_year'],
                                  validate='m:1')
print(f"Successfully matched {len(merged_df)} obserations to assessment records ({100 * (len(merged_df) / len(tax_parcels_gdf))} "
