...
print(f"Dropping {(~has_address_info_mask).sum()} rows missing street address, city, or state ({(~has_address_info_mask).sum() / original_N} percent of original dataset).")
...
print(f"Successfully geocoded {len(result) / len(evictions_df)} percent of remaining dataset. Dropping failed rows.")
...
print(f"Dropping {(~mask).sum()} observations where court_person_type is not \'judge\' ({(~mask).sum() / original_N} percent of original dataset).")
evictions_df = evictions_df.loc[mask, :]

# Drop cases which were resolved via mediation.
mask = evictions_df['disposition_found'] != "Mediated"
print(f"Dropping {(~mask).sum()} observations where disposition_found is \'Mediated\' ({(~mask.sum()) / original_N} percent of original dataset).")
evictions_df = evictions_df.loc[mask, :]
...
print(f"Dropping {(~mask).sum()} observations resolved through voluntary dismissal ({(~mask.sum()) / original_N} percent of original dataset).")
evictions_df = evictions_df.loc[mask, :]

# Drop cases where disposition_found is "Other".
mask = ~(evictions_df['disposition_found'] == "Other")
print(f"Dropping {(~mask).sum()} observations where disposition_found is \'Other\' ({(~mask.sum()) / original_N} percent of original dataset).")
evictions_df = evictions_df.loc[mask, :]
...
print(f"Dropping {(~mask).sum()} observations disposition_found is \'Defaulted\' but judgment_for_pdu is \'Defendant\' ({(~mask.sum()) / original_N} percent of original dataset).")
evictions_df = evictions_df.loc[mask, :]
...
print(f"Dropping {(~mask).sum()} observations disposition_found is \'Dismissed\' but judgment_for_pdu is \'Plaintiff\' ({(~mask.sum()) / original_N} percent of original dataset).")
print(f"Dropping {(~mask).sum()} observations where disposition_found is \'Other\' ({(~mask.sum()) / original_N} percent of original dataset).")
